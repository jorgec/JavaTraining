...
public class AccountDataAccessObject {
	...
	public ArrayList<Account> getUsers() throws SQLException{
		...
		ResultSet uRS = this.get();
		
		while(uRS.next()) {
			Account u = new Account();
			u.setId(uRS.getInt("id"));
			u.setUsername(uRS.getString("username"));
			u.setFirstName(uRS.getString("firstName"));
			u.setLastName(uRS.getString("lastName"));
			u.setRole(uRS.getString("role"));
			users.add(u);
		}
