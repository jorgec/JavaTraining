...
public class ReservationDataAccessObject {
	...
	public ArrayList<Reservation> getReservations() throws SQLException{
		...
		while(rDS.next()) {
			...
			try {
				...
				r.setRoomId(rDS.getInt("roomId"));
				r.setAccountId(rDS.getInt("accountId"));
				r.setStartDate(rDS.getDate("startDate"));
				r.setEndDate(rDS.getDate("endDate"));
				r.setStartTime(rDS.getTime("startDate"));
				r.setEndTime(rDS.getTime("endDate"));
				r.setStatus(rDS.getString("status"));
				reservations.add(r);
			} catch (SQLException e) {
				e.printStackTrace();
			}
	...
	public ArrayList<Reservation> getReservations(Account account) throws SQLException{
		ArrayList<Reservation> reservations = new ArrayList<Reservation>();
		ResultSet rDS = this.get(account);
	...
	public boolean validate(Reservation r) {
		...
		query.append(" ((? >= `startDate`) AND (? <= `endDate`)) OR ");
		query.append(" ((? <= `startDate`) AND (? >= `endDate`)) OR ");
		query.append(" ((? >= `startDate`) AND (? <= `endDate`))) AND ");
		query.append(" `roomId` = ?");
	...
	public boolean createWithString(Reservation r) {
		...
		String startDate = r.getsStartDate(); //new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(r.getStartDate());		
		String endDate = r.getsEndDate(); //new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(r.getEndDate());
		try {
			stmt = conn.prepareStatement(query);
			stmt.setInt(1, r.getRoomId());
			stmt.setInt(2, r.getAccountId());
